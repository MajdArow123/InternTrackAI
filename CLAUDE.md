...
- Run: `dotnet test InternTrackAI.sln`. Current count: **780 tests, all passing** (2026-09-16, ~30 s; the 3 `PostgresMigrationTests` are no-ops unless `INTERNTRACK_PG_CONNECTION` is set). CI runs the same on every push/PR to `main`.
...
- `/Identity/Account/ForgotPassword` is anonymous and now spends real email quota (Resend free tier: 100/day), with no rate limit on it. Worth a per-IP limiter that rejects with the *same* neutral page.
