...
- `ResumeRewriteService.cs` **[AI]** — "Rewrite a bullet": one pasted resume bullet + one application → 2–3 rewrites. `BuildContextAsync` (owner-scoped) reads company, role and JobDescription only — **profile skills are deliberately not an input** (a real-call check showed the model pasting a profile skill onto a bullet that never mentioned it). `BuildPrompt` puts `<application>`, `<job_description>` (3000 chars) and `<bullet>` (400) in tagged sections; `SystemPrompt` = data-only rule + 10 rules: one line under 30 words, past-tense opener, `BannedOpeners` (Responsible for, Worked on, Helped with, Assisted in, Involved in), never invent or recompute a number, a required bracketed placeholder in the Impact-first variant when the bullet states no measurable result, never add technologies/scope, never state an outcome the bullet doesn't state (no "enhancing/improving/streamlining/optimizing/focusing on [thing]" clauses), keep collaborative work collaborative, posting vocabulary only where it names the same thing, no first person, three distinct angles with no two sharing more than half of the longer one's words. Examples in the prompt are placeholders only. Reply contract `{"variants":[{text, angle}]}` (json_object mode needs a top-level object). `Parse` tolerates one fence and a bare array, collapses text to one line, drops blanks/repeats/over 300 chars, and drops any variant failing `InventsNumber` (a number outside `[...]` that the bullet doesn't contain) or `InventsOutcome` (a vague -ing outcome clause naming nothing from the bullet; deliberately lenient — the verb the bullet uses, a placeholder-only clause, or any one bullet word passes it); fewer than 2 left → `BadFormatError`, or `InventedContentError` when a guard caused it. `NormalizeBullet` strips leading glyphs. `DemoVariants` = fixed rewrites of `DemoSampleBullet`. Logs application id, count, tokens only. ProfileController.RewriteBullet.
...
- Run: `dotnet test InternTrackAI.sln`. Current count: **605 tests, all passing** (2026-09-15, ~24 s; the 3 `PostgresMigrationTests` are no-ops unless `INTERNTRACK_PG_CONNECTION` is set). CI runs the same on every push/PR to `main`.
