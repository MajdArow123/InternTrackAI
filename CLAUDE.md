...
- `/Identity/Account/Register` is anonymous, unthrottled and needs no confirmed email, so the per-user `"ai"` bucket can be reset by making a new account. The AI limiter is sound; the supply of identities is not. The cheapest fix is a per-IP registration limit; nothing is in place today.
