...
- `/Identity/Account/Login` uses `lockoutOnFailure: false`, so password guessing is unlimited and each attempt burns a 100k-iteration PBKDF2 hash. `if (result.IsLockedOut)` in the page is dead code.
