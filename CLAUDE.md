...
- `EmailTemplates.cs` — the one place an outgoing email's HTML and plain text are written; a static method per email over a shared `Layout`, deliberately not a templating system. `PasswordReset(resetUrl, lifespan)` takes the real `DataProtectionTokenProviderOptions.TokenLifespan` so the stated expiry can't drift from the token. Inline styles with literal hex mirroring the `site.css` tokens (mail clients can't read CSS custom properties), no image, stylesheet, font or tracking pixel, and **never the literal `http://`** — a bare `<!doctype html>`, no `xmlns`, because `ForwardedProtoTests` asserts the body contains no `http://` at all.
