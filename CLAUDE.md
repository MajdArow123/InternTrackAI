...
- `ResumeRewriteService.cs` **[AI]** — "Rewrite a bullet": one pasted resume bullet + one application → 2–3 rewrites. `BuildContextAsync` (owner-scoped) reads only company, role, JobDescription and the profile's skills. `BuildPrompt` puts `<application>`, `<applicant_skills>`, `<job_description>` (3000 chars), `<bullet>` (400) in tagged sections; `SystemPrompt` = data-only rule + rules: one line under 30 words, past-tense verb opener, `BannedOpeners` (Responsible for, Worked on, Helped with, Assisted in, Involved in), never invent a number (carry originals unchanged, else bracketed placeholders like `[X]%`), never add technologies/scope, posting vocabulary only where it names the same thing, no first person, three angles (Impact first / Technical detail / Concise). Examples in the prompt are placeholders only. Reply contract `{"variants":[{text, angle}]}` (json_object mode needs a top-level object). `Parse` tolerates one fence and a bare array, collapses text to one line, drops blanks/repeats/over 300 chars, and **drops any variant stating a number (outside `[...]`) that the bullet doesn't contain**; fewer than 2 left → `BadFormatError` (or `InventedNumberError` when the guard caused it). `NormalizeBullet` strips leading glyphs. `DemoVariants` = fixed rewrites of `DemoSampleBullet`. Logs application id, count, tokens only. ProfileController.RewriteBullet.
...
- Run: `dotnet test InternTrackAI.sln`. Current count: **593 tests, all passing** (2026-09-15, ~24 s; the 3 `PostgresMigrationTests` are no-ops unless `INTERNTRACK_PG_CONNECTION` is set). CI runs the same on every push/PR to `main`.
